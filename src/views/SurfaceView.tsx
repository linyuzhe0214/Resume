...
import { MapPin, Route, Search, Layers, Split } from 'lucide-react';
import { format } from 'date-fns';
import { cn } from '../utils/utils';
import { KmlPoint, KmlMainlinePoint, KmlRampPoint, KmlIndex } from '../utils/kmlParser';

interface SurfaceViewProps {
  currentTime: Date;
  autoTracking: boolean;
  setAutoTracking: (val: boolean) => void;
  gpsStatus: string;
  accuracy: number | null;
  location: GeolocationPosition | null;
  highwayName: string;
  setHighwayName: (val: string) => void;
  direction: string;
  ...
  mileage: number;
  formatMileage: (meters: number) => string;
  searchMode: 'auto' | 'mainline' | 'ramp';
  setSearchMode: (val: 'auto' | 'mainline' | 'ramp') => void;
  kmlLoading: boolean;
  currentKmlPoint: KmlPoint | null;
  kmlIndex: KmlIndex | null;
}

export function SurfaceView({
  currentTime, autoTracking, setAutoTracking, gpsStatus, accuracy, location,
  highwayName, setHighwayName, direction, setDirection, searchQuery, setSearchQuery,
  handleSearch, mileage, formatMileage, searchMode, setSearchMode, kmlLoading,
  currentKmlPoint, kmlIndex
}: SurfaceViewProps) {
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-40 flex flex-col items-center">
      <div className="responsive-container flex flex-col gap-4 py-4 w-full">
        {/* Header */}
